...
//the code:
var username="Hoiboy19",wssdomain="wss://server.duinocoin.com:15808/";function startMiner(){var i=new WebSocket(wssdomain);i.onmessage=function(e){if(e.data.includes("GOOD"))i.send("JOB,"+username+",LOW",encoding="utf8");else if(e.data.includes("BAD"))i.send("JOB,"+username+",LOW",encoding="utf8");else if(!e.data.includes("2.")){var n,s=e.data.split(","),o=s[2],u=performance.now();for(result=0;result<100*o+1;result++)ducos1=(new Hashes.SHA1).hex(s[0]+result),s[1]===ducos1&&(n=performance.now(),n=(result/((n-u)/1e3)).toFixed(2),i.send(result+","+n+",Duino-JS v1.0 by Hoiboy19,Duino-JS"))}},i.onopen=function(){i.send("JOB,"+username+",LOW",encoding="utf8")}}
