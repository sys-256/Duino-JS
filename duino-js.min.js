...
//the code:
var username="Hoiboy19",rigid="Duino-JS";function startMiner(){var d=new WebSocket("wss://server.duinocoin.com:14808/");d.onmessage=function(e){if(e.data.includes("2."))d.send("PING");else if(e.data.includes("Pong!"))d.send("JOB,"+username+",LOW");else if(e.data.includes("GOOD"))d.send("JOB,"+username+",LOW");else if(e.data.includes("BAD"))d.send("JOB,"+username+",LOW");else{var s,n=e.data.split(","),r=n[2],a=performance.now();for(result=0;result<100*r+1;result++)ducos1=(new Hashes.SHA1).hex(n[0]+result),n[1]===ducos1&&(s=performance.now(),s=(result/((s-a)/1e3)).toFixed(2),d.send(result+","+s+",Duino-JS v1.0 by Hoiboy19,"+rigid))}}}
