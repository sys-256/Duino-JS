...
//the code:
var username="Hoiboy19",wssdomain="wss://server.duinocoin.com:15808/",soc=new WebSocket(wssdomain);soc.onmessage=function(e){if(e.data.includes("GOOD"))soc.send("JOB,"+username+",LOW",encoding="utf8");else if(e.data.includes("BAD"))soc.send("JOB,"+username+",LOW",encoding="utf8");else if(!e.data.includes("2.")){var s,n=e.data.split(","),o=n[2],u=performance.now();for(result=0;result<100*o+1;result++)ducos1=(new Hashes.SHA1).hex(n[0]+result),n[1]===ducos1&&(s=performance.now(),s=(result/((s-u)/1e3)).toFixed(2),soc.send(result+","+s+",Duino-JS v1.0 by Hoiboy19,Duino-JS"))}},soc.onopen=function(){soc.send("JOB,"+username+",LOW",encoding="utf8")};
